...
public class SleepTimerRunner extends Thread {
    ...
    private void startShakeExtend() {
        ...
        SensorManager m_sensorManager = (SensorManager) _service
        ...
        if (deviceSupportsAccelerometer)
            m_sensorManager.registerListener(listener,
    ...
    private void additionalStopSettings() {
        ...
        boolean turnOffBluetooth = settings.getBoolean("pref_stop_bluetooth", false);
        boolean goIntoAirplane = settings.getBoolean("pref_go_airplane", false);
        boolean turnOffNotifications = settings.getBoolean("pref_mute_notifications", false);
    ...
    private void goIntoAirplaneMode() {
        ...
        if (!isEnabled) {
            ...
            Settings.System.putInt(_service.getContentResolver(), Settings.System.AIRPLANE_MODE_ON,
                    isEnabled ? 0 : 1);
    ...
    public void sendPauseBroadcast() {
        ...
        KeyEvent downEvent = new KeyEvent(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(),
        ...
        KeyEvent upEvent = new KeyEvent(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(),
        ...
        if (sdkVersion == 8 && RootUtils.hasRoot(_service, false)) {
            try {
                ...
                if (result != null) {
                    ...
                    String processId = result.substring(spaceIndex - 1, spaceAfter);
    ...
    private List<String> getFixSetApps() {
        ...
        restartMusicApps.add("com.sec.android.app.music"); // Experia Music
                                                           // Player
        ...
        restartMusicApps.add("com.samsung.sec.android.MusicPlayer"); // Samsung
                                                                     // Music
                                                                     // Player
        ...
        restartMusicApps.add("com.bamnetworks.mobile.android.gameday"); // mlb
                                                                        // at
                                                                        // bat
                                                                        // 2010
        restartMusicApps.add("nc.zimme"); // Zimly Media Player
        restartMusicApps.add("com.development.androrb"); // Androrb - Orb on
                                                         // your Mobile
        restartMusicApps.add("com.techfort.audiobooks"); // Audiobooks
                                                         // http://www.androlib.com/android.application.com-techfort-audiobooks-jBpAw.aspx
        ...
        restartMusicApps.add("daapps.media.publicradio"); // Public Radio Live
                                                          // Stream
    ...
    private List<String> getFixSetServices() {
        ...
        restartMusicServices.add("com.snoggdoggler.android.applications.doggcatcher.v1_0");
    ...
    private String getOwnService() {
        // Get own set service
        SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(_service);
        String ownService = settings.getString(SleepTimer.PREFS_OWN_SERVICE, "");
        return ownService;
    }
    ...
    private class SensorShakeListener implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
                ...
                if (totalForce > forceThreshHold) // && (m_totalForcePrev >
                                                  // forceThreshHold)
                {
                    if (enteredOnce)
                        return;
                    ...
                    extendMinutes(shakeMinutesAsInt); // TODO: snoozeMinutes
